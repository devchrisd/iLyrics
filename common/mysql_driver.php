<?php
require_once(DEFAULT_PATH.'/dbi/dbi.php');
...
class mysql_interface_class extends dbi_class
{
   ...
   {
        parent::dbi_class($host,$user,$passwd,$db,$ac);
   }
    ...
    function &connect()
    {
        ...
        while($this->connection == FALSE && --$retry >= 0)
        {
            ...
            if($c)
            {
                ...
                if($this->database != null)
                {
                    ...
                    if(!$r)
                    {
                        ...
                        throw new ic_sql_exception("DBI " .$this->user . "@" .  $this->host . ": $msg  (" . $this->error() . ")", $this->errno());
                    }
                }
            ...
            else
            {
                ...
                throw new ic_sql_exception("DBI failed to connect to " . $this->user . "@" .  $this->host, $this->errno());
            }
        }
     ...
     if($this->connection)
     {
        ...
        else
        {
           $msg = "failed to select database " . $dbname
                 ...
                 . ": " . $this->error();
           debug(__METHOD__ . " - " . $msg);
           throw new ic_sql_exception("DBI: " . $this->user . "@" .  $this->host . ": " . $msg, $this->errno());
        }
     }
     else
     {
   ...
   function last_insert_id()
   {
      ...
      if(!$result)
      {
        throw new ic_sql_exception("DBI " .$this->user . "@" .  $this->host . ": failed to retreive last_insert_id: " . $this->error(), $this->errno());
      }
      return $result;
   }
...
//            $this->trace("failed to connect to " . $this->user . "@" .  $this->host);
      ...
      if($this->connection)
      {
         if (!is_array($supp_err_no))
         {
            ...
            {
                debug('dbi', LOGFAC_USER, LOGLEV_DEBUG, date('Y-m-d H:i:s') . ' - SLOW: ' . $query . '[  ' . round($elapsed, 3) . 's ]');
            }
         }
         else
         {
      ...
      if(!$result)
      {
        ...
        {
            debug('dbi',LOGFAC_USER,LOGLEV_DEBUG,"suppressing exception for error no. $err");
        }
        else
        {
            debug('dbi',LOGFAC_USER,LOGLEV_ERROR,date('Y-m-d H:i:s') . " - " . $this->user . "@" .  $this->host . ": SQL statement failed: " . $this->error() . " `$query''", $this->errno());
            throw new ic_sql_exception("DBI " .$this->user . "@" .  $this->host . ": SQL statement failed: " . $this->error() . " `$query''", $this->errno());
        }
      }
